...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "workouts, journal, Advanced Labs biomarkers, AND Strength Trainer "
        "per-set weight/reps/volume — the current strength source), PushPress "
        "(the gym's programmed workout-of-the-day across class types), Google "
        "Calendar, Cronometer/Cal AI nutrition, and Copilot Money in a single "
        "warehouse. (Hevy is a retired strength source kept for history; its "
        "data ends 2026-06-04 and is unified with Whoop in vw_strength_set.) "
        ...
        "specific-exercise questions PREFER the era-spanning Whoop-lift "
        "tools — get_whoop_lift_progression, get_whoop_lift_prs, "
        "get_whoop_lift_workouts, get_whoop_lift_sets — which read the "
        "unified Hevy+Whoop history (vw_strength_set). The older "
        "get_strength_* / get_exercise_progression / get_rep_maxes tools "
        "see the unified record too but are best for muscle-group volume "
        "breakdowns. fact_workout (Whoop) only carries HR/strain. "
