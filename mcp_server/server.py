...
mcp = FastMCP(
    name="life-os",
    instructions=(
        "Santi's personal health/training/nutrition/finance warehouse.\n"
        "\n"
        "TWO CALLS BEFORE YOU ANSWER ANYTHING ANALYTICAL:\n"
        "  1. get_data_freshness() — which sources are current, which ended, "
        "which are broken. Reading a nutrition gap as 'he barely ate' when the "
        "truth is 'nothing was logged that week' is the most expensive mistake "
        "available here, and this is the only call that tells them apart.\n"
        "  2. get_tool_guide() — which tool answers which question, the "
        "per-source traps, and worked recipes. Cheap, and it is kept current "
        "with the schema.\n"
        "\n"
        "ONE SCHEMA, MANY SOURCES. Every source lands in one canonical set of "
        "tables in SI units (kg, meters, seconds), tagged with its source and "
        "measurement method. Whoop, Garmin, Hevy, Cronometer, Lose It, Cal AI, "
        "Zero and Apple Health all converge. Coverage runs 2023-08-14 to today.\n"
        "\n"
        "PREFER THE UNIFIED TOOLS. get_activity, get_activity_sets, "
        "get_activity_exercises, get_nutrition, get_body_composition, "
        "get_lab_values, get_adherence, get_coverage. They read cross-source, "
        "deduplicated, quality-filtered tables. The older per-source tools "
        "(get_workouts, get_strength_*, get_whoop_lift_*, get_food_log, "
        "get_lab_results) still work but each sees ONE vendor's slice and will "
        "double-count a session or blood draw recorded by two systems.\n"
        "\n"
        "For daily-grain questions use mart_daily — the joins are done and it "
        "carries provenance columns (activity_sources, sleep_source, "
        "nutrition_source, weight_method).\n"
        "\n"
        "HEALTH QUESTIONS (energy, hormones, lipids, sleep, inflammation, "
        "vitamins, libido, weight, recovery) — ground the answer in "
        "get_lab_values before reasoning. get_biomarker_info for one marker.\n"
        "\n"
        "BODY-IMAGE questions have their own surface: get_body_image_summary, "
        "get_body_image_critique, get_body_image_recommendations. Use "
        "correlate_metrics with lag_days for 'does X affect Y'.\n"
        "\n"
        "PushPress = the gym's PROGRAMMED workout, not what he did. Never "
        "present it as performed training.\n"
        "\n"
        "READ AND WRITE. ask_sql for any SELECT; db_describe_table / db_insert "
        "/ db_update / db_upsert / execute_sql for mutations; log_food, "
        "submit_lab_results, log_body_composition and friends when a semantic "
        "tool fits. See get_tool_guide('writing') for the safe flow.\n"
        "\n"
        "Data quality is flagged, never deleted. Read paths exclude flagged "
        "rows by default; pass include_flagged=true to see them with reasons."
    ),
...
@_tool(description=(
    "How to use this MCP well: which tool answers which question, the "
    "per-source traps that produce confidently wrong answers, and worked "
    "recipes. Call this at the start of any analytical session, right after "
    "get_data_freshness. Topics: start_here, tool_selection, gotchas, "
    "writing, recipes — omit `topic` for all of them."
))
def get_tool_guide(topic: str | None = None) -> dict:
    from mcp_server.tool_guide import guide_for

    return U._ok("get_tool_guide", [guide_for(topic)])
